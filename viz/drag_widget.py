...
class DragWidget:
    ...
    def store_W1_fun(self):
        self.store_W1 = True
        self.W1 = None

    def store_W2_fun(self):
        self.store_W2 = True
        self.W2 = None

    def apply_W1_minus_W2(self):
        if self.W1 is None or self.W2 is None:
            return
        self.w_diff = (self.W1 - self.W2).clone().detach()

        self.apply_diff = True

    def apply_W2_minus_W1(self):
        if self.W1 is None or self.W2 is None:
            return
        self.w_diff = (self.W2 - self.W1).clone().detach()
        # self.w_diff.detach()
        self.apply_diff = True
    ...
    def __call__(self, show=True):
        ...
        if show:
            with imgui_utils.grayed_out(self.disabled_time != 0):
                ...
                imgui.text(' ')
                imgui.same_line(viz.label_w)
                if imgui_utils.button("Step once", width=viz.button_w, enabled='image' in viz.result):
                    self.is_drag = True
                    self.drag_once = True
                    if len(self.points) > len(self.targets):
                        self.points = self.points[:len(self.targets)]
                ...
                imgui.text(' ')
                imgui.same_line(viz.label_w)
                if imgui_utils.button('Store W1', width=viz.button_w, enabled='image' in viz.result):
                    self.store_W1_fun()

                imgui.same_line()

                if imgui_utils.button('Store W2', width=viz.button_w, enabled='image' in viz.result):
                    self.store_W2_fun()

                imgui.text(' ')
                imgui.same_line(viz.label_w)
                if imgui_utils.button('Add W2 - W1', width=viz.button_w, enabled='image' in viz.result):
                    self.apply_W2_minus_W1()

                imgui.same_line()

                if imgui_utils.button('Add W1 - W2', width=viz.button_w, enabled='image' in viz.result):
                    self.apply_W1_minus_W2()
        ...
        viz.args.drag_once = self.drag_once
        if self.is_drag:
            self.iteration += 1
