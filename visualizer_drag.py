...
from viz import renderer, visualize_widget
...
class Visualizer(imgui_window.ImguiWindow):
    def __init__(self, capture_dir=None):
        ...
        self.visualize_latent = visualize_widget.VisualizeWidget(self)
    ...
    def draw_frame(self):
        ...
        if self.drag_widget.store_W1:
            # get the W
            W_value = self._async_renderer.renderer_obj.w.clone().detach()
            # then store the W
            self.drag_widget.W1 = W_value
            print(W_value)
            self.drag_widget.store_W1 = False

        if self.drag_widget.store_W2:
            # get the W
            W_value = self._async_renderer.renderer_obj.w.clone().detach()
            # then store the W
            self.drag_widget.W2 = W_value
            print(W_value)
            self.drag_widget.store_W2 = False

        if self.drag_widget.apply_diff:
            # get the W
            W_value = self.drag_widget.w_diff
            # W_value.requires_grad = True
            print("diff applied: ")
            print(W_value)
            # override renderer W
            print(self._async_renderer.renderer_obj.w)
            with torch.no_grad():
                self._async_renderer.renderer_obj.w += W_value
            print(self._async_renderer.renderer_obj.w)
        ...
        if self._image_area is not None:
            ...
            clicked, down, img_x, img_y = imgui_utils.click_hidden_window(
                '##image_area', self._image_area[0], self._image_area[1], self._image_area[2], self._image_area[3],
        ...
        if self.is_skipping_frames() and not self.force_render:
            pass
        elif self._defer_rendering > 0 and not self.force_render:
            ...
            if self.drag_widget.drag_once:
                self.drag_widget.drag_once = False
                self.drag_widget.is_drag = False
        ...
        if 'image' in self.result:
            ...
            if self._tex_img is not self.result.image or True:
            ...
            for point in self.drag_widget.points:
                ...
                rect_size = 2*self.drag_widget.r2 + 1
                gl_utils.draw_rect(pos=np.array([pos_x - round(rect_size / 2), pos_y - round(rect_size / 2)]), size=[rect_size, rect_size],
        ...
        if self._async_renderer.renderer_obj is not None:
                self.visualize_latent.w = self._async_renderer.renderer_obj.w.clone().detach()
                self.visualize_latent.started = True
        self.visualize_latent.w1 = self.drag_widget.W1
        self.visualize_latent.w2 = self.drag_widget.W2
        self.visualize_latent.w_diff = self.drag_widget.w_diff
        # End frame.
        self._adjust_font_size()
        imgui.end()
        self.end_frame()
...
class AsyncRenderer:
    ...
    def set_args(self, search_size, **args):
        # print("set args")
        ...
        if args2 != _cur_args:
            if self._is_async:
                ...
                self._set_args_sync(search_size, **args)
            self._cur_args = args

    def _set_args_async(self, **args):
        print("set args async")
    ...
    def _set_args_sync(self, search_size, **args):
        print("set args sync")
        if self._renderer_obj is None:
            self._renderer_obj = renderer.Renderer()
        self._cur_result = self._renderer_obj.render(search_size=search_size, **args)
    ...
    @staticmethod
    def _process_fn(args_queue, result_queue, search_size):
        print("process fn")
        ...
        while True:
            ...
            if args != cur_args or stamp != cur_stamp:
                result = renderer_obj.render(search_size=search_size, **args)
                if 'error' in result:
